   ...
   Structure of a project:
   {
     ...
     en: {
       ...
       detail: {                         // shown in the click-through modal
         // Any text/caption/cell accepts: **bold**, $math$ (MathJax),
         // [ref:label] and [cite:id] (see below). \n\n starts a new paragraph.
         ...
         // Cross-references and bibliography, LaTeX-style:
         //   - Figures and tables are auto-numbered "Figure N" / "Table N"
         //     (do NOT write the number in the caption). Give one a `label`
         //     and point to it in any text/caption with [ref:label] $-$ it
         //     renders as a link "Figure N" that scrolls to it.
         //   - Cite a source anywhere with [cite:id] (or [cite:id1,id2]) $-$ it
         //     renders as a bracketed number linking to the References section.
         //     e.g.  { src: 'x.png', label: 'fig-x', caption: 'From [cite:smith].' }
         //           { text: 'The setup ([ref:fig-x]) follows [cite:smith].' }
         references: [                  // the bibliography, numbered in order;
           { id: 'smith', text: 'J. Smith et al., Journal ... (2020).', url: 'https://doi.org/...' }
         ]                             // [cite:smith] -> [1]. url is optional.
       }
     },
     fr: { ...same shape... }
   }
  ...
  featured: [
    {
      ...
      en: {
        ...
        detail: {
          methodology:
            '**Geometry.** Every dimension comes from Weiß et al. [cite:weiss2015], Tables 1-2 and sections 2.1-2.4. The origin is placed at the spallation target centre so that each Z coordinate in the input file is directly the distance quoted in the paper. The model includes a pure Pb target and water moderator, the 317 mm beam pipe, the first collimator (1 m of steel, 200 mm bore), the second collimator $-$ 3 m long and layered axially: 2 m of steel, 0.6 m of borated polyethylene, then 0.4 m with a B₄C core, its bore tapering from 70 mm to 21.8 mm $-$ the lead disks that extend the collimation downstream, the concrete shielding, the 4 t stainless-steel sphere fill, and the beam dump (see [ref:fig-geometry]). In absence of referenced values, the external radii of the borated core, C2, and the concrete shielding were set to 7.5 cm, 30 cm, and 70 cm respectively. The steel sphere fill is homogenised at a density that conserves the published 4 t mass rather than a packing fraction.\n\n' +
            '**Source.** A user-defined Fortran source routine emits neutrons from an extended disk on the moderator face. Each neutron is aimed at a uniformly sampled point of the first collimator aperture and carrying the exact solid-angle weight $w = R^2·Δz/(4d^3)$, so that scored quantities normalise to an isotropic $4π$ source. The energy spectrum is a four-component composite $-$ thermal Maxwellian, rising epithermal power law, and two log-normal peaks $-$ shaped to match the published isolethargic flux. This spectrum is an input assumption, not a prediction: comparing it back to the figure it was tuned against would be circular, and the piece says so.\n\n' +
            '**Scoring.** Neutron fluence spectrum at the measurement plane (19.95 m) normalised over the 100 mm diameter surface used in the reference; a 1 mm-resolution radial beam profile; an ambient dose-equivalent H*(10) map with the ICRP-74 conversion set [cite:icrp74]; and residual nuclide inventories plus residual dose maps in the collimator steel at six cooling times after a 180-day irradiation. Per-cycle binary output is merged with the standard FLUKA tools (usxsuw, usbsuw, ustsuw, usrsuw), converted to ASCII, and analysed in Python. A shell driver recompiles the source routine only when it changed, runs the cycles, merges the units and regenerates the figures in one command.',
          /*
          methodology:
          ...
          results: [
            {
              blocks: [
                { heading: 'The geometry' },
                {
                  ...
                  caption: 'R-Z cross-section of the n_TOF EAR2 beam line as modelled in FLUKA. Top: the full 25 m line from the lead spallation target to the beam dump, with the horizontal axis compressed (Z and R are not to the same scale); the dashed line marks the measurement plane at a flight path of 19.95 m. Bottom: the second collimator to scale, showing its axial layering $-$ 2 m of steel, then 0.6 m of borated polyethylene, then 0.4 m with a B₄C core $-$ and the conical bore narrowing from 70 mm to 21.8 mm. All body coordinates are parsed directly from the FLUKA input, so the drawing cannot drift from the simulated geometry. Dimensions follow [cite:weiss2015], Tables 1-2.'
                },
                {
            ...
            },
            {
              blocks: [
                { heading: 'The emitted spectrum' },
                {
                  ...
                  caption: 'Emitted neutron source spectrum in isolethargic units.'
                },
                {
            ...
            },
            {
              blocks: [
                { heading: 'The beam profile $-$ and what it constrains' },
                {
                  ...
                  caption: 'Radial beam profile at the sample position, simulated vs published FWHM.'
                },
                {
                  text: 'The published beam width at the sample position is 21 mm FWHM. This is the one quantity in the model that is genuinely independent of the tuned spectrum of [ref:fig-spectrum]: it is fixed by the collimator geometry and by the size of the emitting region.'
                },
                {
                  text: 'The first model, with a 15.85 cm emitting disk, gave 29.5 mm $-$ 40 % too wide. Rather than tune parameters until the number fell into place, I built an analytic model of the profile as the convolution of the geometric image of the collimator exit with the demagnified source, which showed something the simulation alone would have hidden ([ref:tab-fwhm]):'
                },
                {
                  table: {
                    label: 'tab-fwhm',
                    caption: 'Beam FWHM at the sample position as a function of the effective source radius: analytic convolution model against FLUKA.',
            ...
            },
            {
              blocks: [
                { heading: 'Radiation protection: activation of the collimator' },
                {
                  ...
                  caption: 'Induced activity and residual dose rate in the C2 steel section as a function of cooling time, after a nominal 180-day irradiation at $10^{13}$ n/s. The dominant nuclide, labelled at each point, shifts from short-lived to long-lived as the material cools $-$ the textbook signature of an activation inventory relaxing over time.'
                },
                {
                  text: 'The scoring uses FLUKA\'s RADDECAY chain: a RESNUCLEI inventory and a residual dose map are evaluated at six cooling times from end-of-irradiation to one year. At shutdown the activity is dominated by manganese-56 (2.58 h half-life); within a day it has decayed away and chromium-51 (27.7 d) takes over; after a year the residual is mostly iron-55 (2.74 yr). Total activity falls by a factor ≈18 over the year and the peak residual dose rate by ≈74. The inventory at shutdown is summarised in [ref:tab-activation].'
                },
                {
                  table: {
                    ...
                    rows: [
                      ['Mn-56', '2.58 h', '$2.7 \\times 10^{11}$', '52 %'],
                      ['Cr-51', '27.7 d', '$1.0 \\times 10^{11}$', '20 %'],
                      ['Mo-99', '66 h', '$2.0 \\times 10^{10}$', '4 %'],
                      ['Co-58', '70.9 d', '$2.0 \\times 10^{10}$', '4 %'],
                      ['Mn-54', '312 d', '$1.8 \\times 10^{10}$', '3 %'],
                      ['Fe-55', '2.74 yr', '$1.8 \\times 10^{10}$', '3 %'],
                    ],
                    caption: '190 nuclides in total, $5.2 \\times 10^{11}$ Bq at shutdown.'
                  }
                },
                {
                  text: 'These are the expected activation products of stainless steel under a neutron field (Mn and Cr from iron and chromium, Co from nickel), which is the main reassurance that the inventory is being built correctly. The absolute scale depends on the assumed $10^{13}$ n/s beam intensity, a stated nominal figure rather than a measured one; the relative decay and the nuclide ranking do not.'
                }
              ]
            }
          ],
          references: [
            { id: 'weiss2015', text: 'C. Weiß et al., "The new vertical neutron beam line at the CERN n_TOF facility design and outlook on the performance", Nucl. Instrum. Methods Phys. Res. A 799 (2015) 90-98.', url: 'https://doi.org/10.1016/j.nima.2015.07.027' },
            { id: 'icrp74', text: 'ICRP, Publication 74: Conversion Coefficients for use in Radiological Protection against External Radiation, Ann. ICRP 26 (3-4), 1996.', url: 'https://www.icrp.org/publication.asp?id=ICRP%20Publication%2074' }
          ]
        }
      },
      fr: {
        ...
        detail: {
          ...
          */
          methodology:
            '**Géométrie.** Toutes les cotes viennent de Weiß et al. [cite:weiss2015], Tab. 1-2 et §2.1-2.4. Origine au centre de la cible pour que chaque Z du fichier soit directement la distance citée. Contenu : cible Pb pur + modérateur, tuyau 317 mm, C1 (1 m acier, alésage 200 mm), C2 (3 m, couches axiales 2 m acier / 0.6 m B-PE / 0.4 m à cœur B₄C, alésage 70 → 21.8 mm), disques Pb, blindage béton, 4 t de billes d\'acier, beam dump (voir [ref:fig-geometry]). Étant absentes des valeurs de la référence, les rayons externes du cœur B₄C, du C2, et du blindage en béton ont été mises à 7.5 cm, 30 cm, et 70 cm respectivement. Les billes sont homogénéisées à densité conservant la masse publiée et non le taux d\'empilement.\n\n' +
            '**Source.** Une routine écrite en Fortran émet des neutron depuis un disque à le surface du modérateur. Chaque neutron dirigé vers un point tiré uniformément sur l\'ouverture de C1 avec le poids exact $w = R^2·Δz/(4d^3)$, normalisation 4π. Spectre composite à quatre composantes. Le texte dit explicitement que ce spectre est une hypothèse d\'entrée, pas une prédiction.\n\n' +
            '**Scoring.** Spectre au plan de mesure (19.95 m, convention Ø 100 mm), profil radial à 1 mm, carte H*(10) (ICRP-74 [cite:icrp74]), inventaires de nucléides résiduels et cartes de dose à six temps de refroidissement après 180 j. Fusion par usxsuw/usbsuw/ustsuw/usrsuw, analyse Python, script shell qui enchaîne recompilation conditionnelle → cycles → fusion → figures.',
          results: [
            {
              blocks: [
                { heading: 'La géométrie' },
                {
                  label: 'fig-geometry',
                  src: 'assets/projects/ntof_geometry.png',
                  caption: 'Coupe R-Z de la ligne n_TOF EAR2 telle que modélisée dans FLUKA. En haut : la ligne complète sur 25 m, de la cible de spallation en plomb au beam dump, l\'axe horizontal étant comprimé (Z et R ne sont pas à la même échelle) ; le trait tireté marque le plan de mesure à 19.95 m de parcours. En bas : le second collimateur à l\'échelle, montrant son empilement axial $-$ 2 m d\'acier, puis 0.6 m de polyéthylène boré, puis 0.4 m à cœur de B₄C $-$ et l\'alésage conique se resserrant de 70 mm à 21.8 mm. Les coordonnées de tous les corps sont lues directement dans le fichier d\'entrée FLUKA : le tracé ne peut donc pas diverger de la géométrie simulée. Cotes d\'après [cite:weiss2015], Tableaux 1-2.'
                },
                {
            ...
            },
            {
              blocks: [
                { heading: 'Le spectre émis' },
                {
                  label: 'fig-spectrum',
                  src: 'assets/projects/ntof_flux_lethargy.png',
                  caption: 'Spectre neutronique source émis, en unités isoléthargiques.'
                },
                {
            ...
            },
            {
              blocks: [
                { heading: 'Le profil de faisceau $-$ et ce qu\'il contraint' },
                {
                  label: 'fig-beam',
                  src: 'assets/projects/ntof_beam_fwhm.png',
                  caption: 'Profil radial du faisceau à la position échantillon, FWHM simulée vs publiée.'
                },
                {
                  text: 'La largeur de faisceau publiée à la position échantillon est de 21 mm FWHM. C\'est la seule grandeur du modèle réellement indépendante du spectre ajusté de [ref:fig-spectrum] : elle est fixée par la géométrie de collimation et par la taille de la région émettrice.'
                },
                {
                ...
                },
                {
                  table: {
                    label: 'tab-fwhm',
                    caption: 'FWHM du faisceau à la position échantillon en fonction du rayon de source effectif : modèle analytique par convolution face à FLUKA.',
              ...
              blocks: [
                ...
                },
                {
                  table: {
                    ...
                    rows: [
                      ['Mn-56', '2.58 h', '$2.7 \\times 10^{11}$', '52 %'],
                      ['Cr-51', '27.7 j', '$1.0 \\times 10^{11}$', '20 %'],
                      ['Mo-99', '66 h', '$2.0 \\times 10^{10}$', '4 %'],
                      ['Co-58', '70.9 j', '$2.0 \\times 10^{10}$', '4 %'],
                      ['Mn-54', '312 j', '$1.8 \\times 10^{10}$', '3 %'],
                      ['Fe-55', '2.74 ans', '$1.8 \\times 10^{10}$', '3 %'],
                    ],
                    caption: '190 nucléides au total, $5.2 \\times 10^{11}$ Bq à l\'arrêt.'
                  }
                },
                {
                  text: 'Ce sont les produits d\'activation attendus de l\'acier inox sous flux neutronique (Mn et Cr issus du fer et du chrome, Co du nickel), ce qui constitue la principale assurance que l\'inventaire est correctement construit. L\'échelle absolue dépend de l\'intensité supposée de $10^{13}$ n/s, valeur nominale déclarée et non mesurée ; la décroissance relative et le classement des nucléides, eux, n\'en dépendent pas.'
                }
              ]
            }
          ],
          references: [
            { id: 'weiss2015', text: 'C. Weiß et al., « The new vertical neutron beam line at the CERN n_TOF facility design and outlook on the performance », Nucl. Instrum. Methods Phys. Res. A 799 (2015) 90-98.', url: 'https://doi.org/10.1016/j.nima.2015.07.027' },
            { id: 'icrp74', text: 'CIPR, Publication 74 : Conversion Coefficients for use in Radiological Protection against External Radiation, Ann. ICRP 26 (3-4), 1996.', url: 'https://www.icrp.org/publication.asp?id=ICRP%20Publication%2074' }
          ]
        }
      }
    }
...
window.renderProjects = function (lang) {
  ...
  const labels = {
    ...
    method: i18n.sec_method || 'Methodology',
    results: i18n.sec_results || 'Results & figures',
    references: i18n.sec_references || (L === 'fr' ? 'Références' : 'References'),
    figure: i18n.figure || 'Figure',
    table: i18n.table || (L === 'fr' ? 'Tableau' : 'Table'),
  ...
  /* ---- LaTeX-style cross-references and citations ----------------------
     Figures and tables are auto-numbered in document order. Give one a
     `label` and refer to it in any text/caption with [ref:label] — it
     renders as a hyperlink "Figure N" / "Table N" that scrolls to it.
     Bibliography: `detail.references` is an array of { id, text, url? };
     cite a source anywhere with [cite:id] (or [cite:id1,id2]), which
     renders as a bracketed number linking to the References section. */
  let xrefLabels = {};   // label   -> { word, number, domId }
  let xrefCites = {};     // cite id -> { number, domId }
  let figCounter = 0, tabCounter = 0;

  function slugify(s) {
    return String(s).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'x';
  }
  const figDomId = (b, n) => 'pm-fig-' + (b.label ? slugify(b.label) : n);
  const tabDomId = (t, n) => 'pm-tab-' + (t.label ? slugify(t.label) : n);

  // Pass 1: number every figure/table/reference and record its DOM id, so a
  // [ref:...] or [cite:...] resolves even when it points forward (a figure
  // mentioned before it appears, the bibliography rendered at the very end).
  function indexXrefs(results, references, lbl) {
    xrefLabels = {}; xrefCites = {};
    let f = 0, t = 0;
    (results || []).forEach(r => {
      const blocks = Array.isArray(r.blocks) ? r.blocks : [r];
      blocks.forEach(b => {
        if (b && b.src) { f++; if (b.label) xrefLabels[b.label] = { word: lbl.figure, number: f, domId: figDomId(b, f) }; }
        if (b && b.table) { t++; if (b.table.label) xrefLabels[b.table.label] = { word: lbl.table, number: t, domId: tabDomId(b.table, t) }; }
      });
    });
    (references || []).forEach((ref, i) => {
      const key = ref.id || ('ref' + (i + 1));
      xrefCites[key] = { number: i + 1, domId: 'pm-ref-' + slugify(key) };
    });
  }

  // Escape, then turn **bold**, [cite:...] and [ref:...] markers into markup.
  // Runs after escaping (the markers contain no HTML-special characters), so
  // surrounding prose stays safe and $...$ math is left untouched for MathJax.
  function renderInline(text) {
    let s = escapeHtml(text == null ? '' : String(text));
    // **bold** -> <strong> (done first so a bolded [ref:]/[cite:] still resolves)
    s = s.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
    s = s.replace(/\[cite:([^\]]+)\]/g, (_, keys) => {
      const links = keys.split(',').map(k => k.trim()).filter(Boolean).map(k => {
        const c = xrefCites[k];
        return c ? `<a class="pmodal__cite" href="#${c.domId}" data-xref="${c.domId}">${c.number}</a>` : '?';
      });
      return '[' + links.join(', ') + ']';
    });
    s = s.replace(/\[ref:([^\]]+)\]/g, (_, label) => {
      const r = xrefLabels[label.trim()];
      return r ? `<a class="pmodal__xref" href="#${r.domId}" data-xref="${r.domId}">${r.word}&nbsp;${r.number}</a>` : '??';
    });
    return s;
  }

  // Marker-free, tag-free version for alt text.
  function plainText(text) {
    return String(text == null ? '' : text)
      .replace(/\*\*([^*]+)\*\*/g, '$1')
      .replace(/\[cite:[^\]]+\]/g, '')
      .replace(/\[ref:([^\]]+)\]/g, (_, l) => { const r = xrefLabels[l.trim()]; return r ? r.word + ' ' + r.number : ''; })
      .replace(/\$([^$]*)\$/g, '$1').trim();
  }
  ...
  // block is itself an object with figure (src + caption + label), text
  // (+ heading) and/or table keys; within one block those render figure → text
  // → table.
  function buildBlock(b, lbl) {
    if (!b) return [];
    const pieces = [];

    if (b.src) {
      figCounter++;
      const domId = figDomId(b, figCounter);
      const numTag = `<strong class="pmodal__fig-num">${escapeHtml(lbl.figure)}&nbsp;${figCounter}</strong>`;
      const cap = b.caption ? `${numTag} $-$ ${renderInline(b.caption)}` : numTag;
      pieces.push(`
      <figure id="${domId}" class="pmodal__figure">
        <img src="${escapeAttr(b.src)}" alt="${escapeAttr(plainText(b.caption))}" loading="lazy"
    ...
    if (b.text || b.heading) {
      ...
      <div class="pmodal__note">
        ${b.heading ? `<h4 class="pmodal__note-title">${renderInline(b.heading)}</h4>` : ''}
        ${b.text ? paragraphs(b.text) : ''}
      </div>`);
    }

    const table = buildTable(b.table, lbl);
  ...
  function buildFigures(results, lbl) {
    ...
    figCounter = 0; tabCounter = 0;
    // Everything stacks vertically at the width of the text column.
    const figs = results.map(r => {
      const blocks = Array.isArray(r.blocks) ? r.blocks : [r];
      const pieces = blocks.reduce((acc, b) => acc.concat(buildBlock(b, lbl)), []);
  ...
  /* Table: { head: ['A','B'], rows: [['1','2'], ...], caption: '...',
              align: ['left','right'], label: 'tab-xxx' }
     head, caption, align and label optional; auto-numbered "Table N".
     Cells accept $...$ math and [ref:]/[cite:] markers. */
  function buildTable(t, lbl) {
    if (!t || !t.rows || !t.rows.length) return '';
    tabCounter++;
    const domId = tabDomId(t, tabCounter);
    const align = t.align || [];
    const cls = i => align[i] ? ` class="is-${escapeAttr(align[i])}"` : '';
    const cells = (row, tag) => row
      .map((c, i) => `<${tag}${cls(i)}>${renderInline(c)}</${tag}>`).join('');
    ...
    const numTag = `<strong class="pmodal__fig-num">${escapeHtml(lbl.table)}&nbsp;${tabCounter}</strong>`;
    const cap = t.caption ? `${numTag} $-$ ${renderInline(t.caption)}` : numTag;

    return `
      <div id="${domId}" class="pmodal__table-wrap">
  ...
  // Bibliography rendered as a numbered list; [cite:id] links land here.
  function buildReferences(references, lbl) {
    if (!references || !references.length) return '';
    const items = references.map((ref, i) => {
      const key = ref.id || ('ref' + (i + 1));
      const domId = 'pm-ref-' + slugify(key);
      const link = ref.url
        ? ` <a class="pmodal__ref-link" href="${escapeAttr(ref.url)}" target="_blank" rel="noopener" aria-label="Open source">↗</a>`
        : '';
      return `<li id="${domId}" class="pmodal__ref-item" value="${i + 1}">${renderInline(ref.text)}${link}</li>`;
    }).join('');
    return `
      <section class="pmodal__section">
        <h3 class="pmodal__section-title">${escapeHtml(lbl.references)}</h3>
        <ol class="pmodal__refs">${items}</ol>
      </section>`;
  }
  ...
  window.fillProjectModal = function (idx, lng, lbl) {
    ...
    // Number figures/tables/references first so [ref:] and [cite:] resolve
    indexXrefs(det.results, det.references, lbl);
    ...
    body.innerHTML =
      ...
      buildReferences(det.references, lbl) +
...
function ensureModal() {
  ...
  modal.addEventListener('click', (e) => {
    // A [ref:]/[cite:] link: scroll to its target inside the modal and flash it
    const jump = e.target.closest('[data-xref]');
    if (jump) {
      e.preventDefault();
      const target = modal.querySelector('#' + jump.getAttribute('data-xref'));
      if (target) {
        target.scrollIntoView({ behavior: 'smooth', block: 'center' });
        target.classList.remove('pmodal__flash');
        void target.offsetWidth;   // restart the animation if re-triggered
        target.classList.add('pmodal__flash');
      }
      return;
    }
